...
print("Processing with Dual-Model Architecture...")
...
while True:
    ...
    # ========================================================================
    # PASS 0: WAGON POLYGON (Using best(4).pt)
    # ========================================================================
    wagon_poly = None
        ...
        for box, track_id, cls in zip(results[0].boxes.xyxy, results[0].boxes.id, results[0].boxes.cls):
            ...
            if class_id == 2 and not wagon_detected: 
                ...
                if wagon_poly is not None and cv2.pointPolygonTest(wagon_poly, center, False) >= 0:
                    ...
                    # --- FAST STABILIZATION ---
                    # LOWERED BUFFER SIZE TO 15 SO IT ALWAYS SAVES FAST
                    buffer_size = 15 
...
# If the video ended before OCR could lock, save it as UNKNOWN
